...
public class PersonDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        // получаем пользоваетля из таблицы с формы аутентификации
        Optional<Person> person = personRepository.findByLogin(username);

        // проверяем найден пользователь или нет
        if (person.isEmpty()){
            // выбрасываем исключение сто данный пользователь не найден
            // сключение будет поймано Spring Security и сообщение будет выведено на страницу
